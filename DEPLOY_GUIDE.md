...
會員可直接用 Email + 密碼建立網站帳號並登入，並可在「帳號安全」修改密碼。登入後可線上查看最新/進行中/歷史訊號，使用起訖日期查詢，維護訂閱品種，建立轉帳或線上付款訂單；每筆訂單都會提供「訂單明細」頁，可回看付款狀態、條款同意時間與流程紀錄。下單前需同意服務條款與交易風險揭露，若已設定 Stripe secrets，會員也會看到線上付款按鈕。

Telegram 會員也可用 `/myorders` 查詢最近訂單，或用 `/receipt 訂單ID` 取得單筆訂單明細；建立訂單、付款通知、取消、確認與退款後都會提供對應的訂單明細按鈕。管理員可在後台按「退款」，也可用 `/refund 訂單ID 金額 原因` 記錄人工退款並選擇是否同步停用會員權限。
